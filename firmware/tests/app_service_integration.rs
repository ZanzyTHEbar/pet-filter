...
use petfilter::app::ports::{ActuatorPort, ConfigError, ConfigPort, EventSink, SensorPort, StoragePort, StorageError};
...
struct MockHw { calls: Vec<ActCall>, uvc_on: bool }
impl MockHw {
    fn new() -> Self { Self { calls: Vec::new(), uvc_on: false } }
}
impl SensorPort for MockHw {
    ...
    fn read_ammonia_fast(&mut self) -> f32 { 0.0 }
}

impl ActuatorPort for MockHw {
    fn set_pump(&mut self, duty: u8, forward: bool) { self.calls.push(ActCall::SetPump { duty, forward }); }
    fn stop_pump(&mut self) { self.calls.push(ActCall::SetPump { duty: 0, forward: true }); }
    fn enable_uvc(&mut self, duty: u8) { self.uvc_on = duty > 0; self.calls.push(ActCall::EnableUvc { duty }); }
    fn disable_uvc(&mut self) { self.uvc_on = false; self.calls.push(ActCall::EnableUvc { duty: 0 }); }
    fn fault_shutdown_uvc(&mut self, _reason: &'static str) { self.uvc_on = false; self.calls.push(ActCall::EnableUvc { duty: 0 }); }
    fn is_uvc_on(&self) -> bool { self.uvc_on }
    fn set_led(&mut self, r: u8, g: u8, b: u8) { self.calls.push(ActCall::SetLed { r, g, b }); }
    fn all_off(&mut self) { self.uvc_on = false; self.calls.push(ActCall::AllOff); }
}

struct MockNvs { store: HashMap<String, Vec<u8>> }
impl MockNvs {
...
impl StoragePort for MockNvs {
    ...
    fn write(&mut self, ns: &str, key: &str, data: &[u8]) -> Result<(), StorageError> {
        self.store.insert(format!("{}::{}", ns, key), data.to_vec()); Ok(())
    }
    ...
    fn delete(&mut self, ns: &str, key: &str) -> Result<(), StorageError> {
        self.store.remove(&format!("{}::{}", ns, key)); Ok(())
    }
}
impl ConfigPort for MockNvs {
    fn load(&self) -> Result<SystemConfig, ConfigError> { Ok(SystemConfig::default()) }
    fn save(&self, _c: &SystemConfig) -> Result<(), ConfigError> { Ok(()) }
}

struct LogSink { events: Vec<String> }
impl LogSink { fn new() -> Self { Self { events: Vec::new() } } }
impl EventSink for LogSink {
    fn emit(&mut self, e: &AppEvent) { self.events.push(format!("{:?}", e)); }
}
...
fn start_scrub_leaves_idle_and_commands_actuators() {
    ...
    assert_ne!(app.state(), StateId::Idle, "FSM should leave Idle after StartScrub");
...
fn update_config_marks_dirty() {
    ...
    cfg.pump_duty_percent = 75;
    app.handle_command(AppCommand::UpdateConfig(cfg), &mut hw, &mut sink);
    assert!(app.is_config_dirty(), "dirty flag must be set after UpdateConfig");
}
...
fn auto_save_fires_after_dirty_timeout() {
    ...
    assert!(saved, "auto_save should fire after enough ticks with dirty config");
}
