...
impl NvsAdapter {
        ...
        F: FnOnce(nvs_handle_t) -> Result<T, i32>,
    {
        ...
        let ret = unsafe {
            nvs_open(ns_buf.as_ptr() as *const _, mode, &mut handle)
        };
        if ret != ESP_OK {
            return Err(ret);
        }
...
fn validate_config(cfg: &SystemConfig) -> Result<(), ConfigError> {
    ...
    if cfg.pump_duty_percent > 100 {
        return Err(ConfigError::ValidationFailed("pump_duty_percent must be 0–100"));
    }
    if cfg.uvc_duty_percent > 100 {
        return Err(ConfigError::ValidationFailed("uvc_duty_percent must be 0–100"));
    }
    if !(40.0..=120.0).contains(&cfg.max_temperature_c) {
        return Err(ConfigError::ValidationFailed("max_temperature_c must be 40.0–120.0"));
    }
    if !(10..=600).contains(&cfg.purge_duration_secs) {
        return Err(ConfigError::ValidationFailed("purge_duration_secs must be 10–600"));
    }
    ...
    if !(5..=3600).contains(&cfg.telemetry_interval_secs) {
        return Err(ConfigError::ValidationFailed("telemetry_interval_secs must be 5–3600"));
    }
    Ok(())
}
...
impl ConfigPort for NvsAdapter {
    ...
    fn save(&self, config: &SystemConfig) -> Result<(), ConfigError> {
        ...
        #[cfg(not(target_os = "espidf"))]
        {
            ...
            let store = unsafe {
                &mut *(&self.store as *const HashMap<String, Vec<u8>>
...
impl StoragePort for NvsAdapter {
    fn read(&self, namespace: &str, key: &str, buf: &mut [u8]) -> Result<usize, StorageError> {
        #[cfg(not(target_os = "espidf"))]
        {
            ...
            match self.store.get(&composite) {
                Some(data) => {
    ...
    fn write(&mut self, namespace: &str, key: &str, data: &[u8]) -> Result<(), StorageError> {
        ...
        {
            let composite = Self::composite_key(namespace, key);
            self.store.insert(composite, data.to_vec());
            Ok(())
        }
    ...
    fn exists(&self, namespace: &str, key: &str) -> bool {
        ...
        {
            let composite = Self::composite_key(namespace, key);
            self.store.contains_key(&composite)
        }
...
mod tests {
    ...
    fn rejects_activate_below_range() {
        ...
        cfg.nh3_activate_threshold_ppm = 0.5;
        assert!(matches!(validate_config(&cfg), Err(ConfigError::ValidationFailed(_))));
    }

    #[test]
    fn rejects_deactivate_above_activate() {
        let mut cfg = SystemConfig::default();
        cfg.nh3_activate_threshold_ppm = 10.0;
        cfg.nh3_deactivate_threshold_ppm = 15.0;
        assert!(matches!(validate_config(&cfg), Err(ConfigError::ValidationFailed(_))));
    }
